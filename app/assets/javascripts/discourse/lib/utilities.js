  ...
  **/
  ...
  avatarImg: function(options) {
    var size = Discourse.Utilities.translateSize(options.size),
        rawSize = Discourse.Utilities.getRawSize(size),
        cssClass = "avatar" + (options.extraClasses ? " " + options.extraClasses : ""),
        title = (options.title) ? Handlebars.Utils.escapeExpression(options.title || "") : "";

    return (options.avatarTemplate || "").replace(/\{size\}/g, size)
                                         .replace(/\{raw_size\}/g, rawSize)
